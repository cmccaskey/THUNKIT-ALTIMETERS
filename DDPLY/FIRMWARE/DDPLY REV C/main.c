...
#define launchTrigger 20 //feet in 200mS
...
void SPI_Master_Init(void) {
  DDRA |= (1 << DDA4) | (1 << DDA6) | (1 << DDA3) | (1 << DDA7);
...
int main() {
  ...
  while (1) {
    ...
    if ((UCSR0A & (1 << RXC0))) {
      ...
      if (UDRData == 'v') { //enable verbose logging
        uart0_putchar('v');
        uart0_putchar('\n');
        verboseLogging = true;
      }
    ...
    if (!launch) {
      if (altitudeNew > altitudeOld + launchTrigger) {
        ...
        launch = true;
        int i;
        for (i = 12; i < 512; i += 2) {  //"scrub" EEPROM
          eeprom_write_word((uint16_t * ) i, (uint16_t) 1000);
        }
        eeprom_write_word((uint16_t * ) 12, (uint16_t) altitudeOld); //log old altitude
      }
    }

    if (launch && altitudeNew < altitudeOld){ //apogee detect
        PORTB |= (1 << PB2); //ematch 1 turn on
    }
    ...
    if (launch && eepromIndex < 512) { //data logging to EEPROM
